  ...
  <div class="row content">
    ...
    <div class="col-sm-8 text-center"> 
      <div id="container">
          ...
          <h1><i class="fas fa-book"></i>NUEVO LIBRO</h1>
          <p>Bienvenido, para registrar un libro en SIBITEC <i class="far fa-registered"></i> deberás llenar el siguiente formulario.</p>
          <p><i class="fas fa-check"></i> Puedes dejar los campos vacíos que no se encuentren en el libro.</p>
          <p><i class="fas fa-exclamation"></i> Recuerda <i class="fas fa-exclamation"></i> Si hay varios ejemplares con los mismos datos, se debe de dar de alta uno por uno ya que el folio es único para cada libro.</p>
          <div class="underline">
          </div>
          <form action="../php/vNLibro.php" method="POST" id="form_NLibro" onsubmit="return validarLibro()">
            <div class="name">
              <label for="name"></label>
              <input type="text" placeholder="Número de Ejemplares" name="num_ejemplares" id="num_ejemplares">
            </div>
            <div class="email">
              <label for="name"></label>
              <input type="text" placeholder="Clasificación" name="clasificacion" id="clasificacion">
            </div>
            <div class="telephone">
              <label for="name"></label>
              <input type="text" placeholder="Título del Libro" name="titulo" id="titulo">
            </div>
            <div class="telephone">
              <label for="name"></label>
              <input type="text" placeholder="Autor(es)" name="autores" id="autores">
            </div>
            <div class="telephone">
              <label for="name"></label>
              <input type="text" placeholder="Editorial" name="editorial" id="editorial">
            </div>
            <div class="name">
              <label for="name"></label>
              <input type="text" placeholder="Fecha de Publicación" name="f_publicacion" id="f_publicacion">
            </div>
            <div class="email">
              <label for="name"></label>
              <input type="text" placeholder="Lugar de Publicación" name="l_publicacion" id="l_publicacion">
            </div>
            <div class="name">
              <label for="name"></label>
            ...
            <div class="email">
              <label for="name"></label>
              <input type="text" placeholder="Edición" name="edicion" id="edicion">
            </div>
            <div class="name">
              <label for="name"></label>
              <input type="text" placeholder="Tomo" name="tomo" id="tomo">
            </div>
            <div class="email">
              <label for="name"></label>
              <input type="text" placeholder="Folio" name="folio" id="folio">
            </div>
            <div class="subject">
              <label for="subject"></label>
              <select placeholder="Categoria" name="categoria" id="categoria">
                <option disabled hidden selected>Selecciona Categoría</option>
                <?php
                  include "../php/conexion.php";
                  $categoria = "SELECT nombre FROM categorias ORDER BY nombre";
                  //$cat = $_POST['categoria'];
                  $resultado = mysqli_query($obj_conexion, $categoria);
            ...
            <div class="subject">
              <label for="subject"></label>
              <select placeholder="Seccion" name="seccion" id="seccion">
                <option disabled hidden selected>Selecciona Sección</option>
                <?php
                  include "../php/conexion.php";
                  $seccion = "SELECT nombre FROM secciones ORDER BY nombre";
                  $resultado = mysqli_query($obj_conexion, $seccion);
                  while($row  = mysqli_fetch_assoc($resultado)){
                  ?>
                  <option><?php echo $row["nombre"]; ?></option>
                  <?php } 
...
<script src="../js/vLibro.js"></script>
