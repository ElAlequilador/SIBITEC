...
function Header()
{
    ...
    $var_selectPA = "SELECT * FROM prestamos_docentes WHERE nombre = '$nombre'";
    ...
    $var_telefono = "SELECT telefono FROM docentes WHERE nombre = '$nombre'";
    $res_telefono = mysqli_query($obj_conexion, $var_telefono);
    $nr_telefono = mysqli_fetch_array($res_telefono);

    $var_correo = "SELECT correo FROM docentes WHERE nombre = '$nombre'";
    $res_correo = mysqli_query($obj_conexion, $var_correo);
    $nr_correo = mysqli_fetch_array($res_correo);

    $var_dominio = "SELECT dominio FROM docentes WHERE nombre = '$nombre'";
    ...
    $this->Cell(70,8,'',0);
    $this->Cell(50,8,utf8_decode('Reporte del Docente ' .$nombre),0,0,'C');
    $this->Ln(8);
    $this->Ln(15);
    ...
    $this->Cell(40,8,utf8_decode($nombre.'.'),0,0,'C');
    $this->Ln(5);
...
$var_select = "SELECT folio, titulo, autor, editorial, fecha_salida, fecha_entrega FROM prestamos_docentes
               WHERE nombre = '$nombre'";
$resultado = mysqli_query($obj_conexion, $var_select);

$var_select2 = "SELECT folio, titulo, autor, editorial, fecha_salida, fecha_entrega FROM prestamos_docentes
                WHERE nombre = '$nombre' AND folio = '$folio'";
$res2 = mysqli_query($obj_conexion, $var_select2);


$var_retraso = "SELECT DATEDIFF(NOW(), fecha_entrega) FROM prestamos_docentes WHERE nombre = '$nombre' 
AND folio = '$folio'";
