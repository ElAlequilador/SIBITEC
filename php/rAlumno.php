...
function Header()
{
    ...
    $var_selectPA = "SELECT * FROM prestamos_alumnos WHERE nombre = '$nombre'";
    ...
    $var_con = "SELECT control FROM alumnos WHERE nombre = '$nombre'";
    $res_control = mysqli_query($obj_conexion, $var_con);
    $nr_control = mysqli_fetch_array($res_control);
    
    $var_telefono = "SELECT telefono FROM alumnos WHERE nombre = '$nombre'";
    $res_telefono = mysqli_query($obj_conexion, $var_telefono);
    $nr_telefono = mysqli_fetch_array($res_telefono);

    $var_correo = "SELECT correo FROM alumnos WHERE nombre = '$nombre'";
    $res_correo = mysqli_query($obj_conexion, $var_correo);
    $nr_correo = mysqli_fetch_array($res_correo);

    $var_dominio = "SELECT dominio FROM alumnos WHERE nombre = '$nombre'";
    $res_dominio = mysqli_query($obj_conexion, $var_dominio);
    $nr_dominio = mysqli_fetch_array($res_dominio);

    $var_carrera = "SELECT carrera FROM alumnos WHERE nombre = '$nombre'";
    $res_carrera = mysqli_query($obj_conexion, $var_carrera);
    $nr_carrera = mysqli_fetch_array($res_carrera);
    ...
    $this->Cell(70,8,'',0);
    $this->Cell(50,8,utf8_decode('Reporte del Alumno ' .$nombre),0,0,'C');
    $this->Ln(8);
    $this->Ln(15);
    ...
    $this->Cell(50,8,utf8_decode($nombre.'.'),0,0,'C');
    $this->Ln(5);
    $this->Cell(15,8,utf8_decode('N° Control: '),0,0,'C');
    $this->Cell(20,8,utf8_decode($nr_control[0]),0,0,'C');
    $this->Ln(5);
    $this->Cell(10,8,utf8_decode('Carrera: '),0,0,'C');
    $this->Cell(50,8,utf8_decode($nr_carrera[0]),0,0,'C');
    $this->Ln(5);
...
$nombre = $_POST['nombre'];
$folio = $_POST['folio'];    
$var_select = "SELECT folio, titulo, autor, editorial, fecha_salida, fecha_entrega FROM prestamos_alumnos
               WHERE nombre = '$nombre'";
$resultado = mysqli_query($obj_conexion, $var_select);

$var_select2 = "SELECT folio, fecha_salida, fecha_entrega FROM prestamos_alumnos
                WHERE nombre = '$nombre' AND folio = '$folio'";
$res2 = mysqli_query($obj_conexion, $var_select2);


$var_retraso = "SELECT DATEDIFF(NOW(), fecha_entrega) FROM prestamos_alumnos WHERE nombre = '$nombre' 
AND folio = '$folio'";
