...
function Header()
{
    ...
    $this->Cell(70,8,'',0);
    $this->Cell(50,8,'Reporte de Secciones ',0,0,'C');
    $this->Ln(8);
    $this->Ln(15);

    $this->SetFont('Arial','',8);
    //$this->Cell(70,8,'',0);
    $this->Cell(65,8,'Secciones registradas en SIBITEC a la fecha de hoy. ',0,0,'C');
    $this->Ln(8);

    $this->SetFont('Arial','B',8);

    $this->Cell(5,10, utf8_decode('ID'), 1, 0, 'C', 0);
	$this->Cell(70,10, utf8_decode('Nombre de la Sección'), 1, 0, 'C', 0);
	$this->Cell(50,10, utf8_decode('Categoría'), 1, 0, 'C', 0);
    $this->Cell(45,10, utf8_decode('Fecha de Registro'), 1, 1, 'C', 0);
}
...
$fechaE = $_POST['fechaE'];
$fechaS = $_POST['fechaS'];
$var_select = "SELECT * FROM secciones WHERE fecha_registro BETWEEN '$fechaE' AND '$fechaS' 
               ORDER BY id_seccion";
...
while ($row = mysqli_fetch_assoc($resultado)) {
	$pdf->Cell(5,10, utf8_decode($row['id_seccion']), 1, 0, 'C', 0);
	$pdf->Cell(70,10, utf8_decode($row['nombre']), 1, 0, 'C', 0);
	$pdf->Cell(50,10, utf8_decode($row['categoria']), 1, 0, 'C', 0);
	$pdf->Cell(45,10, utf8_decode($row['fecha_registro']), 1, 1, 'C', 0);
}
$var_select = "SELECT * FROM secciones 
